...
// TestTrueNode_Golden is a SELF-VALIDATING regression test.
//
// The golden data (golden_true_lunarnodes.json) was generated by
// generate_true_lunarnodes.py, which implements the same r×v algorithm
// as goeph's TrueNode(): h = r×v, n = ecliptic_pole × normalize(h),
// longitude = atan2 of n projected onto the ecliptic. Both sides use
// DE440s positions and velocities, the same obliquity constants, and
// the same cross-product/normalization steps — just in different
// languages (Go vs Python/NumPy).
//
// This means the test can only catch language-level bugs (e.g. wrong
// array indexing, operator precedence) but NOT algorithmic errors that
// are shared by both implementations. For true independent validation,
// see TestTrueNode_GoldenOsculating.
...
// TestTrueNode_GoldenOsculating validates TrueNode against an INDEPENDENT
// reference: Skyfield's osculating_elements_of() with ECLIPJ2000 reference frame.
//
// Why this is independent validation (not self-validating):
//
// The golden data (golden_true_lunarnodes_osculating.json) was generated by
// generate_true_lunarnodes_osculating.py using Skyfield's osculating_elements_of()
...
// into the ecliptic frame before computing elements. goeph stays in ICRF and
// converts at the end. A bug in either coordinate transform, normalization
// order, or cross-product operand order would cause disagreement.
//
// Measured agreement: ~4e-11 degrees (~0.1 microarcsecond), which is machine
// epsilon for float64. This confirms goeph's TrueNode is correct.
func TestTrueNode_GoldenOsculating(t *testing.T) {
	eph := loadEphemeris(t)

	data, err := os.ReadFile("../testdata/golden_true_lunarnodes_osculating.json")
	if err != nil {
		t.Fatal(err)
	}
	var golden goldenLunarNodes
	...
	if failures > 0 {
		t.Errorf("%d true node (osculating) failures out of %d tests", failures, len(golden.Tests))
	}
	t.Logf("max diff vs osculating: north=%.2e deg, south=%.2e deg", maxDiffN, maxDiffS)
}
